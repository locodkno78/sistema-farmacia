import {
  ...
  const updateTable = (querySnapshot) => {
    ...
    // Convertir snapshot a array y ordenar alfabéticamente
    const productosArray = [];
    querySnapshot.forEach(doc => {
      productosArray.push({ id: doc.id, ...doc.data() });
    });
    productosArray.sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
    ...
    productosArray.forEach(p => {
      html += `
        <tr>
          ...
          <td>${p.drug}</td>
          <td>
            <button class="btn btn-success button-view" data-id="${p.id}" data-bs-toggle="modal" data-bs-target="#viewProduct">
              <i class="fas fa-eye"></i>
            </button>
            <button class="btn btn-warning button-edit" data-id="${p.id}" data-bs-toggle="modal" data-bs-target="#editProducto">
    ...
    clientesTable.querySelectorAll(".button-view").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        const data = await getProduct(e.currentTarget.getAttribute("data-id"));
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    if (editForm) {
      ...
      console.error("No se encontró #edit-form en el DOM");
    }
  });
